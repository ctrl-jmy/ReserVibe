(function() {
  ...
  const bookingDiv = document.getElementById('booking-view');
  const aboutDiv = document.getElementById('about-view');
  ...
  async function loadAboutContent() {
    ...
    try {
      ...
      aboutContentDiv.innerHTML = html;
      aboutLoaded = true;
    } catch (err) {
      console.error('Failed to load about.html:', err);
      aboutContentDiv.innerHTML = '<div class="text-center text-red-400">Failed to load about content. Make sure about.html exists.</div>';
    }
  }
  ...
  function switchToView(view) {
    ...
    if (view === 'dashboard') dashboardDiv?.classList.remove('hidden');
    if (view === 'booking') bookingDiv?.classList.remove('hidden');
    if (view === 'about') {
      aboutDiv?.classList.remove('hidden');
      if (!aboutLoaded) loadAboutContent();
    }
  }
  ...
  function updateStats() {
    ...
    totalBookingsSpan.innerText = allBookings.length;
    const counts = {};
    ...
    availableDaysSpan.innerText = uniqueDays - fullDaysCount;
  }
  ...
  function renderCalendar() {
    ...
    document.getElementById('prev-month')?.addEventListener('click', () => {
      let newMonth = currentCalendarMonth - 1, newYear = currentCalendarYear;
      if (newMonth < 0) { newMonth = 11; newYear--; }
      currentCalendarYear = newYear; currentCalendarMonth = newMonth; renderCalendar();
    });
  ...
  showRegisterLink.addEventListener('click', () => { loginForm.classList.add('hidden'); forgotForm.classList.add('hidden'); registerForm.classList.remove('hidden'); });
  forgotPasswordBtn.addEventListener('click', () => { loginForm.classList.add('hidden'); registerForm.classList.add('hidden'); forgotForm.classList.remove('hidden'); });
  backToLoginFromReset.addEventListener('click', () => { forgotForm.classList.add('hidden'); loginForm.classList.remove('hidden'); });
  backToLoginFromRegister.addEventListener('click', () => { registerForm.classList.add('hidden'); loginForm.classList.remove('hidden'); });
